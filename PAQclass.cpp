...
#define PROGNAME "PAQclass"  // Please change this if you change the program.
...
private:
  ...
  int code(int i=0) {
    ...
    if (y) {
      entropy += log2(p/4096.0);
    } else {
      entropy += log2((4096.0 - p) / 4096.0);
    }
    ++entropy_count;
...
//        printf("%s already exists\n", archiveName.c_str()), quit();
    ...
    if (mode==DECOMPRESS) {
      ...
      rewind(archive);
      size_t ignore = fread(&filenames[0], 1, header_size, archive);
      fname.resize(files);
      fsize.resize(files);
      char* p=&filenames[0];
    ...
    if (mode==COMPRESS) {
      for (int i=0; i<files; ++i) {
        if (i < test_index) {
          ...
          if (id == 0) {
            compress(fname[i], fsize[i], en);
            printf("%s\t%f\n", fname[i], (-8 * entropy / entropy_count));
            return 0;
          } else {
            int status;
            waitpid(id, &status, 0);
